const _fetchGeoNames = async (zip = '325204') => {
  ...
  const _weather = async (lat, long, time) => {
    const url2 = `http://localhost:8081/weather?latitude=${lat}&longitude=${long}&time=${time}`;
    return await fetch(url2).then(response => {
      return response.json();
    });
  };
  
  const handleWeather = (time, daySinceStart) => {
    const lat = document.getElementById('latitude').textContent;
    const lng = document.getElementById('longitude').textContent;
  
    _weather(lat, lng, time).then(response => {
